...
interface ExamplesPanelProps {
  ...
  conversionMode: 'sqlserver-to-postgres' | 'postgres-to-sqlserver';
}

const ExamplesPanel: React.FC<ExamplesPanelProps> = ({ onSelectExample, conversionMode }) => {
  const { isDarkMode } = useTheme();
  const examples = conversionMode === 'sqlserver-to-postgres' ? sqlExamples : postgresExamples;
  ...
  return (
    <div className={`mb-4 rounded-lg p-4 ${
      ...
      <h3 className="text-sm font-medium mb-2">
        {conversionMode === 'sqlserver-to-postgres' 
          ? 'SQL Server to PostgreSQL Examples:' 
          : 'PostgreSQL to SQL Server Examples:'}
      </h3>
