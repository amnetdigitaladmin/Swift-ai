...
interface WorkflowContextType {
  ...
  addArtifact: (
    artifact: Omit<Artifact, "id" | "createdAt" | "projectId">
  ) => void;
...
export const WorkflowProvider = ({ children }: WorkflowProviderProps) => {
  ...
  const [projects, setProjects] = useState<Project[]>([
    ...
    },
    {
      ...
      description:
        "Designing user interfaces for a secure mobile banking application",
  ...
  const createProject = (
    ...
    const newProject: Project = {
      ...
      projectPriority: metadata?.projectPriority,
  ...
  const getProjectsAssignedToTeam = (teamRole: string) => {
    return projects.filter((project) =>
      project.assignedTeams?.includes(teamRole)
    );
  };
